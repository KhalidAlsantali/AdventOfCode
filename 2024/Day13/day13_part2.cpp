...
vector<vector<int64_t>> mulMat(vector<vector<int64_t>>& m1, vector<vector<int64_t>>& m2){
    ...
    vector<vector<int64_t>> result(r1, vector<int64_t>(c2, 0));
...
int main() {
    ...
    while (getline(inputFile, line)) {
        if (line.find("Button A") != string::npos) {
            ...
            layer_output[0][0] = prize[0] + 10000000000000;
            layer_output[1][0] = prize[1] + 10000000000000;
    ...
    for(size_t i = 0; i < input.size(); ++i){
        ...
        if(result[0][0] % det != 0 || result[1][0] % det != 0){
            continue;
        }
        sum += (x*3) + y;
    }
